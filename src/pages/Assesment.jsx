  ...
  const saveRecord = async () => {
    try {
      if (videoRef.current && videoRef.current.srcObject) {
        ...
        if (update) {
          const response = await axios.patch(`https://be-cbt.trisakti.ac.id/records/${record.question_id}/${record.package_question_id}`, {
            user_id: 1,
          ...
          const response = await axios.post(`https://be-cbt.trisakti.ac.id/records`, {
