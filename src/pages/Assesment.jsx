    ...
    try {
      ...
      if (activePackage) {
        ...
        const responseQuestions = await axios.get(
          ...
          }/questionusers/packagequestion/${data?.package_question_id}/${
            data.user_id
  ...
  const getRecord = async (question, pkg, noreg) => {
    await axios
      .get(
        `${
          import.meta.env.VITE_APP_API_BASE_URL
        }/records/question/${question}/${pkg}/${noreg}`,
        {
          headers: {
            "api-key": "b4621b89b8b68387",
          },
  ...
  const changeQuestion = async (id, packageQuestion, noreg) => {
    try {
      const responseQuestions = await axios.get(
        `${
          import.meta.env.VITE_APP_API_BASE_URL
        }/questionusers/${id}/${packageQuestion}/${noreg}`,
        {
          headers: {
            "api-key": "b4621b89b8b68387",
          },
        ...
        if (update) {
          const response = await axios.patch(
            `${import.meta.env.VITE_APP_API_BASE_URL}/records/${
              record?.question_id
            }/${record?.package_question_id}/${regNumber}`,
            {
              user_id: 1,
          ...
          if (questionActive.package?.type_of_question === "Multiple choice") {
            payload = {
              ...
              package_question_id:
                record?.package_question_id ||
                questionActive?.package_question_id,
              user_id: identityNumber || "",
              registration_number: regNumber,
    ...
    const startCamera = async () => {
      try {
        ...
        const response = await axios.get(
          `${
            import.meta.env.VITE_APP_API_BASE_URL
          }/packagequestionusers/check/${packageParse?.package_question_id}`,
          {
            headers: {
              "api-key": "b4621b89b8b68387",
            },
        ...
        <section className="order-2 md:order-1 w-full md:w-9/12 bg-white shadow-md p-10 h-screen md:h-full md:rounded-3xl md:overflow-y-auto">
            ...
            answersActive.length !== 0 && (
              <div className="mb-2 space-y-2">
                <h1 className="text-xl font-bold">
                  ...
                  <b className="mx-2">
                    waktu {questionActive?.group_questions?.duration} menit
                  </b>
                </h1>
                <p className="text-sm tracking-wide">
                  {questionActive?.group_questions?.naration
                    ?.replace(/<[^>]*>/g, "")
                    ?.replace(/&nbsp;/g, " ")
                    ?.replace(/"/g, "")
                    ?.trim()}
          ...
          <div className="space-y-8">
            <div className="space-y-3">
              {questionActive?.image?.data.length > 0 && (
                <div className="overflow-auto">
                  <img
                    src={`${
                      import.meta.env.VITE_APP_API_BASE_URL
                    }/questions/image/${questionActive.id}`}
                    alt="Question Image"
              ...
              <div className="h-full grid grid-cols-10 md:grid-cols-5 gap-2 overflow-y-auto">
                {questions?.map((question, index) => (
                  <button
                    ...
                    className={`flex items-center justify-center w-10 h-10 rounded-xl border-2 font-bold transition-all ease-in-out ${
                      question.answered
                        ? "bg-emerald-500 text-white border-emerald-400"
                        : "bg-gray-100 text-sky-800 hover:text-white hover:bg-sky-700 border-sky-600"
                    }`}
                  >
                    {index + 1}
                  </button>
