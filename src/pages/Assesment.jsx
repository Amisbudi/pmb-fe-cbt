...
const Assesment = () => {
  ...
  const getQuestions = async () => {
    ...
    try {
      ...
      if (activePackage) {
        ...
        const responseQuestions = await axios.get(`http://10.50.1.149:3000/questionusers/packagequestion/${data.package_question_id}/${data.user_id}`);
  ...
  const getRecord = async (question, pkg) => {
    await axios.get(`http://10.50.1.149:3000/records/question/${question}/${pkg}`)
      .then((response) => {
  ...
  const getAnswers = async (id) => {
    try {
      const responseAnwers = await axios.get(`http://10.50.1.149:3000/answers/question/${id}`);
  ...
  const changeQuestion = async (id, packageQuestion) => {
    try {
      const responseQuestions = await axios.get(`http://10.50.1.149:3000/questionusers/${id}/${packageQuestion}`);
  ...
  const saveRecord = async () => {
    try {
      if (videoRef.current && videoRef.current.srcObject) {
        ...
        if (update) {
          const response = await axios.patch(`http://10.50.1.149:3000/records/${record.question_id}/${record.package_question_id}`, {
            user_id: 1,
          ...
          const response = await axios.post(`http://10.50.1.149:3000/records`, {
            ...
            <div className='space-y-3'>
              {
                ...
                <img
                  src={`http://10.50.1.149:3000/questions/image/${questionActive.id}`}
                  alt="Question Image"
                  ...
                  answersActive.map((answer, index) =>
                    <div key={index} className="flex items-center p-4 border border-gray-200 hover:bg-gray-50 transition ease-in-out rounded-2xl">
                      ...
                      <label htmlFor={`answer-${answer.id}`} className="w-full py-4 ms-2 text-sm font-medium text-gray-900">{answer.name}</label>
                      {
                        answer.image &&
                        <img
                          src={`http://10.50.1.149:3000/answers/image/${answer.id}`}
                          alt="Answer Image"
