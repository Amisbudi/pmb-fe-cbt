...
const Assesment = () => {
    ...
    try {
      ...
      if (activePackage) {
        ...
        const responseQuestions = await axios.get(
          `${import.meta.env.REACT_APP_API_BASE_URL}/questionusers/packagequestion/${data?.package_question_id}/${data.user_id}`,
          {
            headers: {
  ...
  const getRecord = async (question, pkg) => {
    await axios
      .get(
        `${import.meta.env.REACT_APP_API_BASE_URL}/records/question/${question}/${pkg}`,
        {
          headers: {
            "api-key": "b4621b89b8b68387",
          },
  ...
  const getAnswers = async (id) => {
    try {
      const responseAnwers = await axios.get(
        `${import.meta.env.REACT_APP_API_BASE_URL}/answers/question/${id}`,
        {
          headers: {
            "api-key": "b4621b89b8b68387",
          },
  ...
  const changeQuestion = async (id, packageQuestion) => {
    try {
      const responseQuestions = await axios.get(
        `${import.meta.env.REACT_APP_API_BASE_URL}/questionusers/${id}/${packageQuestion}`,
        {
          headers: {
            "api-key": "b4621b89b8b68387",
          },
      ...
      if (videoRef.current && videoRef.current.srcObject) {
        ...
        if (update) {
          const response = await axios.patch(
            `${import.meta.env.REACT_APP_API_BASE_URL}/records/${record?.question_id}/${record?.package_question_id}`,
            {
              user_id: 1,
          ...
          const response = await axios.post(
            '${import.meta.env.REACT_APP_API_BASE_URL}/records',
            payload,
            {
              headers: {
                'api-key': 'b4621b89b8b68387',
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        const response = await axios.get(`${import.meta.env.REACT_APP_API_BASE_URL}/packagequestionusers/check/${packageParse?.package_question_id}`,{
          headers: {
            "api-key": "b4621b89b8b68387",
          },
        });
          ...
          <div className="space-y-8">
            <div className="space-y-3">
              {questionActive?.image && (
                <img
                  src={`${import.meta.env.REACT_APP_API_BASE_URL}/questions/image/${questionActive.id}`}
                  alt="Question Image"
              ...
              {questionActive?.package?.type_of_question !== 'Essay' &&
                ...
                answersActive.map((answer, index) => (
                  ...
                  >
                    ...
                    {answer.image && (
                      <img
                        src={`${import.meta.env.REACT_APP_API_BASE_URL}/answers/image/${answer.id}`}
                        alt="Answer Image"
